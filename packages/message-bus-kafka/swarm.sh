...
function initialize_package() {
  ...
  # Create the kafka_public network if it doesn't exist
  if ! docker network ls | grep -q "kafka_public"; then
    log info "Creating kafka_public network"
    docker network create --driver overlay --attachable kafka_public || true
  fi
